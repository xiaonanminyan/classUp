...
class index extends foreground {
	...
	public function register() {
		...
		if(isset($_POST['dosubmit'])) {
			...
			$userinfo['username'] = (isset($_POST['username']) && $_POST['username']) ? $_POST['username'] : exit('0');   //gai
			...
			if(pc_base::load_config('system', 'phpsso')) {
				...
				$userid = $this->db->insert($userinfo, 1);//往数据库写数据
				//showmessage('注册成功','/index.php?m=member&c=index&a=login');
				if($userid > 0) {
						//执行登陆操作
		...
		if(isset($_POST['dosubmit'])) {
			...
			if(pc_base::load_config('system', 'phpsso')) {
			    ...
			    if ($r['password'] == $password) {
                    ...
                    showmessage('登录失败', 'index.php?m=member&c=index&a=login');
                }
			} 
			else {
				//密码错误剩余重试次数
